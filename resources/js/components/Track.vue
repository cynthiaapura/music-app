<template>
  <div class="max-w-sm rounded shadow-lg">
    ...
    <div class="w-full flex justify-content-between">
      <Link v-if="$page.props.auth.isAdmin"  :href="route('tracks.edit', { track: track })" 
      class="bg-green-500 hover:bg-green-700 text-white font-bold py-2 px-4 me-2 rounded w-full mb-1 text-center">
        Modifier
      </Link>
      <Link v-if="$page.props.auth.isAdmin"  :href="route('tracks.destroy', { track: track })" 
      method="delete" as="button" preserve-scroll 
      class="bg-red-500 hover:bg-red-700 text-white font-bold py-2 px-4 rounded w-full mb-1 text-center">
        Supprimer
      </Link>
    </div>
